...
const statusConfig: Record<string, { label: string; className: string }> = {
  PENDING: { label: "Pending — Awaiting Client Review", className: "bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-700" },
...
export default function AdminApprovalDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm("Delete this deliverable? This action cannot be undone.")) return;
    setDeleting(true);
  ...
  const handleResubmit = async (e: React.FormEvent) => {
    ...
    if (revisionFile) formData.append("file", revisionFile);
  ...
  return (
    <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                For: <span className="font-medium text-foreground">{deliverable.user.companyName || deliverable.user.name}</span>
                {deliverable.user.companyName && <span className="ml-1">({deliverable.user.name})</span>}
              </p>
      ...
      {/* Submit revision — only shown when changes are requested */}
      {isChangesRequested && (
        <Card className="mb-6 border-amber-200 dark:border-amber-800">
          <CardHeader className="pb-3">
            ...
            <p className="text-sm text-muted-foreground">
              The client has requested changes. Upload a new version and add a note.
            </p>
          </CardHeader>
          <CardContent className="pt-0">
            <form onSubmit={handleResubmit} className="space-y-3">
              ...
              <div>
                ...
                >
                  ...
                  <span className="text-sm text-muted-foreground truncate">
                    {revisionFile ? revisionFile.name : "Click to upload a revised file"}
                  </span>
                  <input
                    id="revision-file"
                    type="file"
              ...
              <Button type="submit" disabled={submitLoading} className="gap-1.5">
                <Upload className="h-4 w-4" />
                {submitLoading ? "Submitting..." : `Submit v${deliverable.version + 1}`}
              </Button>
            </form>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="pt-0 space-y-3">
          <textarea
            ...
            placeholder="Add an internal note or message to the client..."
            rows={3}
            className="w-full border border-input rounded-md px-3 py-2 text-sm bg-background text-foreground resize-none"
          />
          <Button
